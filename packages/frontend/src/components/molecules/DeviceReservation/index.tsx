...
const DeviceReservation: React.FC = () => {
    ...
    const handleSave = async (deviceId: string) => {
        try {
            ...
            const { actualBorrowTime, actualReturnTime, deviceCondition } = values;
            ...
            await axios.put(
                `http://localhost:3009/api/v1/reservations/${selectedRecord.reservationId}`,
                {
            ...
            await axios.post(
                `http://localhost:3009/api/v1/reservation-histories`,
                {
                    reservationId: selectedRecord.reservationId,
                    userId: selectedRecord.user.userName,
                    deviceId: selectedRecord.device.deviceId,
                    actualBorrowTime,
                    actualReturnTime: actualReturnTime || null,
                },
    ...
    const approveByLecturer = async (reservationId: string, lecturerId: string) => {
        try {
            ...
            await axios.put(
                `http://localhost:3009/api/v1/reservations/${reservationId}/approve-lecturer`,
                { lecturerId },
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
    ...
    const approveByAdmin = async (reservationId: string, deviceId: string) => {
        ...
        try {
            ...
            await axios.put(
                `http://localhost:3009/api/v1/reservations/${reservationId}/approve-admin`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
    ...
    const handleDelete = async (reservationId: string) => {
        try {
            ...
            await axios.delete(`http://localhost:3009/api/v1/reservations/${reservationId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const rejectReservation = async (reservationId: string, deviceId: string) => {
        try {
            ...
            await axios.put(
                `http://localhost:3009/api/v1/reservations/${reservationId}`,
    ...
    const columns: TableColumnsType<DeviceReservationType> = [
        ...
        },
        {
            ...
            key: 'lecturerId',
            render: (lecturerId) => lecturerId?.userName || 'Không xác định',
        },
        {
            title: 'Tên thiết bị',
            dataIndex: 'device',
