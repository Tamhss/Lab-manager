...
export class BorrowHistoryService {
  ...
  async create(data: CreateBorrowHistoryDto) {
    return this.prisma.borrowHistory.upsert({
  ...
  async findOne(id: string) {
    return this.prisma.borrowHistory.findUnique({ where: { borrowHistoryId: id } });
  }

  async update(id: string, data: UpdateBorrowHistoryDto) {
    return this.prisma.borrowHistory.update({ where: { borrowHistoryId: id }, data });
  }

  async remove(id: string) {
    return this.prisma.borrowHistory.delete({ where: { borrowHistoryId: id } });
  }
}
